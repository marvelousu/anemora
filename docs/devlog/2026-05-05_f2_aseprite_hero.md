...
- Initial pass used deterministic local pixel processing because Aseprite was not installed yet. Follow-up validation used Aseprite `1.3.17.2-x64` to re-export the final PNGs as indexed palette-v0 sprites.
...
- Hero front's androgynous expression still needs user judgment against the silent-protagonist indie target reference.
