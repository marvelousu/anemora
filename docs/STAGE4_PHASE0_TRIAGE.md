...
| 4 | Character v2 redraw + Resident review | Resident_A C3 sizing follow-up ready | User review + Codex asset work | Hero and Resident_B are now the accepted visual reference. Resident_A v2 remains imported, but runtime review found its pixel feel too strong and its face/head scale too large against the Hero; C was directionally right but still large-headed, so C2/C3 sizing comparisons were added. | Current recommendation is C3 as the base plus a final 1px head/hair reduction during 32x48 extraction, unless user requests another regeneration; targeted character tests and full PlayMode remain green after import. |
...
| Version | Date | Change |
|---|---|---|
| v0.5 | 2026-05-06 | Records Resident_A A/B/C follow-up review sheet as the next user gate before runtime replacement. C sizing follow-up now identifies C3 as the best base with final head/hair reduction still recommended. |
