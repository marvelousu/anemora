...
| 4 | Character v2 redraw + Resident review | Resident_A F2 review ready | User review + Codex asset work | Hero and Resident_B are now the accepted visual reference. Resident_A v2 remains imported, but runtime review found its pixel feel too strong and its face/head scale too large against the Hero. C3 fix C was demoted because its head/body connection reads detached; G was then demoted because the hair is too short and drifts away from the preferred F direction. F2/F3/F4 were generated from F's longer-hair direction. | F2 is the strongest current review candidate because it keeps F-like hair and mood while reducing the silhouette to `bbox 16x45`. Do not runtime-import until user approves; F3/F4 also keep the hair but read taller / more forceful. |
...
| Version | Date | Change |
|---|---|---|
| v0.8 | 2026-05-07 | Records user preference for F over G, adds F2/F3/F4 F-based comparison, and identifies F2 as strongest current review candidate. |
