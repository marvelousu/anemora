...
- 上記 §0.1 の 1 → 6 が **連続して破綻なく動く** (ノートPC での 60 FPS 維持、デスクトップで見栄え確認)
...
**手順**:
1. ノートPC (統合 Radeon) で Editor 起動 → 60 FPS 維持確認
2. Windows ビルド版で同シーンを再生、Editor との差を記録
3. デスクトップ (RTX 2070S) で見栄え確認 (色、影、ポータル境界の質感)
...
ノート PC で E0 / E2 / E3 を進め、E4 以降は描画負荷が読めないため **デスクトップ への移動を視野に入れる** (STAGE3_PLAN §10.2 切替トリガー)。
...
| ID | 項目 | 必須 | 環境 | 確認方法 |
|---|---|---|---|---|
| V1 | Stencil 描画が破綻しない (Z-fighting / 縁にじみ) | ✓ | ノートPC + デスクトップ | E1 Sandbox + 通し |
| V2 | HD-2D Tier 2 動的影と非干渉 | ✓ | ノートPC + デスクトップ | E1 + 通し |
| V3 | ポータル境界の往復で擦り抜けない | ✓ | ノートPC | E4 単体 + 通し |
| V4 | 反転後の collider が正しい側だけ有効 | ✓ | ノートPC | E4 単体 |
| V5 | ActionRecord の record / replay が成立 | ✓ | ノートPC | E5 通し |
| V6 | reflected フラグで二重反映が起きない | ✓ | ノートPC | E5 単体 |
| V7 | ノートPC で 60 FPS 維持 | ✓ | ノートPC | E6 計測 |
| V8 | Editor とビルド版で動作差なし | ✓ | ノートPC | E6 |
| V9 | Camera_Past の同期で位置ずれなし | ✓ | ノートPC | E2 + 通し |
| V10 | シンボルグレーアウト 2 つが選択不可 | ✓ | ノートPC | E3 |
| V11 | 入力ナビゲーション (KB / Pad) で破綻なし | ✓ | ノートPC | E3 |
| V12 | 反転フレームの白フェードが派手すぎない | ✓ | デスクトップ | E4 + デスクトップ確認 |
| V13 | デスクトップ で見栄え確認 | ✓ | デスクトップ | E6 |
| V14 | URP Volume Profile 切替の必要性判定 | △ | デスクトップ | E6 (差分が小さければ不要、ADR-0002) |
| V15 | ポータル枠の AI VFX 投入余地 | △ | デスクトップ | E6 後段、F トラックで素材投入 |
...
| ノートPC で 60 FPS 出ない | E1 段階で 30 FPS 切る | デスクトップ へ即移行、ノートPC はビルド検証用に位置づけ。STAGE3_PLAN §10.2 切替トリガーを発火 |
