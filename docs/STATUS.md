...
- **point15 レンダラループ / 環境アップリフト**: 2026-06-24 最新レビュー build で Time Window aperture を frame behind の depth-aware composite にし、CC0 textured tree subset + imported nature scale uplift + realistic nearfield nature clusters + authored under-canopy foliage + dark-pixel leaf alpha keying + all-map foreground wild-grass layer + all-map realistic tree grove layer + realistic distant panorama depth layer + ScenicRelief forest/ridge layer + close/mid realistic specimen tree framing layer を導入。黒い副作用が強かった photo vegetation card 案は accepted path から外した。全マップ capture、ValidateHouseSliceBatch、AssetValidation、EditMode renderer freeze、BuildAndValidateBatch、player smoke 済み。レビュー packet は `docs/review/2026-06-24T23-43_realistic_specimen_trees_r2/`。
