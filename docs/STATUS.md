...
- **point15 レンダラループ / 環境アップリフト**: 2026-06-23 最新レビュー build で Time Window aperture を depth-aware AlphaTest/LEqual にし、CC0 textured tree subset + deterministic undergrowth companions + imported nature scale uplift + deterministic broadleaf saplings + photo vegetation ground layer + nature leaf-tone lift を導入。黒い副作用が強かった photo branch/canopy card 案は破棄。全マップ capture、ValidateHouseSliceBatch、AssetValidation、EditMode renderer freeze、BuildAndValidateBatch、player smoke 済み。レビュー packet は `docs/review/2026-06-23T22-59_realistic_nature_leaf_lift_r1/`。
