...
> **Status (2026-05-04)**: v0 breadth-first 起草中。Stage 1 で確定した方針 (時の窓 / 観測者輪廻 / ベール剥離 / HD-2D Tier 2) を実装観点で再展開し、Stage 2 オープン要件 (主人公設定・衰退原因・フレーム細部仕様・シンボル段階開示・能動行動機構・ゾーン構成・カメラ設計) は **TBD として明示**して Stage 3 (Vertical Slice) で確定する方針。
>
> 改訂履歴は §13.4 参照。
...
| 公開段階 | GitHub Public (Day 0) → itch.io (VS 完成後) → Steam (オプション) |
...
主人公 (TBD: 名前 / 性別 / 年齢、§4.1) は時の筆を授かり、衰退を食い止めるため過去/未来への干渉を始める。各層で「衰退は止められる」「衰退は別の場所に押しつけているだけ」「衰退は観測者の罪」と認識が更新され、第 5 層で **観測者が世界の正体だった** という主語反転に至る。
...
**名前: TBD** (Stage 3 ヒーロービジュアル試作と同期して確定)。`docs/STAGE3_TBD_RESOLUTION.md` §1 で Stage 3 A トラック /spec の確定事項を集約。

| 属性 | 内容 |
|---|---|
| 性別 | **中性的・両読み可能** (Frisk / Yume Nikki 系統)。Stage 3 ヒーロービジュアル試作で微調整余地あり |
| 年齢層 | 仮置き 10 代後半〜20 代前半、ヒーロービジュアル試作で微調整 |
| 沈黙設定 | **Silent protagonist** (Undertale 型)。主人公はしゃべらない、感情のみテキストウィンドウで表現 |
| 出身 | 第 1 ゾーンの街の住人 (偽記憶設定。家族構成・職業の細部は VS では不要、Stage 4 で確定) |
...
- 現在の年齢までの記憶は偽記憶、自覚はない (家族の記憶も含む)
- 偽記憶のレベル: ぼんやりと存在を覚えている程度、顔や名前は鮮明に出てこない

> `feedback_anemora_no_premature_lockin.md` 遵守 — 名前 / ヒーロービジュアル v1 は Stage 3 試作と同期して確定。Stage 3 A トラック確定事項の詳細は `docs/STAGE3_TBD_RESOLUTION.md` 参照。
...
|---|---|---|
| 家族 (ぼんやり覚えている存在) | 主人公の表層動機の核、過去への干渉対象 (失われた家族の記憶) | 第 2 層で能動行動の代償が顕在化、最終盤で偽記憶と判明 |
...
- 主人公性別 TBD (Stage 3 で確定)。プレイヤー選択制は採用しない (物語の主語反転に整合させるため固定)
...
### 6.2 ゾーン候補 (TBD: Stage 3 で確定)

Stage 1 で議論したゾーン仮配置。**A〜D は仮称**であり、配置方針 (4-6 連結ゾーン + 真層対応) のみ Stage 2 で固定し、テーマ・順序は Stage 3 で差し替え可能:

| ID | 仮称 | テーマ (仮) | 主要違和感 (仮) | 想定プレイ時間 |
|---|---|---|---|---|
| A | 仮称: 街 | 第 1 ゾーン候補、主人公の出身地 | 失われた住人・本・歌 | 1-2 h |
...
| Stage 5 (β) | Day 19-25 | バランス調整・バグ修正・itch.io リリース準備 |
| Stage 6 (公開) | Day 26-28 | itch.io 公開 / Steam 判断 |
...
| 23 | β 品質に達しているか | itch.io のみ公開 / Steam 見送り / プレイ時間 4-6h → 2-4h |
...
| 音響 | AIVA + Suno + Stable Audio + Reaper |
| 最終判断 / ディレクション | ユーザー |
...
| 2 | itch.io ($0) | Day 25-28 (Vertical Slice 完成後) |
| 3 | Steam ($100 サンクコスト判断後) | Stage 5 完了後オプション |
...
| リスク | 影響 | 軽減策 |
|---|---|---|
| GitHub Public で先行公開のリスク | 低 | ライセンス明記 (TBD: MIT / CC-BY-NC / 独自) で対応 |
...
#### キャラクター
- [x] 主人公の性別 (中性的・両読み可能) / 年齢 (10 後半-20 前半 仮置き) / silent protagonist / 動機 3 層 / 真相 — Stage 3 A トラック /spec で確定 (`docs/STAGE3_TBD_RESOLUTION.md` §1)
- [ ] 主人公の名前 (Stage 3 ヒーロービジュアル試作と同期して確定)
- [ ] 主人公の出身詳細 (家族構成 / 職業の細部) — VS では不要、Stage 4 で確定
- [ ] 主要登場人物 5-8 人の個別シート化 (§4.2)

#### 物語
- [x] 時の筆 = 古代の遺物、主人公は元から持っていた、史上主人公だけが使える — Stage 3 A トラックで確定
- [x] 主人公の真相 (突然変異的に発生した異物、ループを止めるための存在、偽記憶) — Stage 3 A トラックで確定
- [ ] 衰退原因の具体描写 (比喩か実体か、複数案並列維持、Stage 4 で確定)
- [ ] 主人公の創造主体 (世界か観測者か、Stage 4-5 で確定)
...
- [ ] カメラ角度 / ズーム挙動 (§7.4)
- [ ] アセット数規模の確定 (§7.6)
- [ ] 主人公ヒーロービジュアル (Stage 3、`PITCH.md` §1)
